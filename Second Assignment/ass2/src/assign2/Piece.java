...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		width ++;
		height ++;
		Arrays.sort(body, new TPointComparator());

		generateSkirt ();
	}

	/**
	 * Comparator class for TPoints
	 * Since it is needed only for TPoint array sorting
	 * there is no need to define it as independent class,
	 * inner class will do :)
	 */

	private class TPointComparator implements Comparator <TPoint> {
	...
	/**
	 * Generates skirt array for this piece, always called
	 * by constructor
	 */

	private void generateSkirt () {
		skirt = new int[width];
		Arrays.fill(skirt, MAXNUM);
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for (int i = 0; i < body.length; i ++) {
			newBody[i] = new TPoint(0, 0);
			//Rotation matrix multiplication
			newBody[i].x = cos90*body[i].x - sin90*body[i].y;
			newBody[i].y = sin90*body[i].x + cos90*body[i].y;

			//Adjust point horizontally
			newBody[i].x+=height-1;
		}
	...
	*/	
	...
	/**
	 ...
	 My note!
	 For every Piece constructor calls sort function for it's body
	 Hence if the mismatch is detected for any equal indices then
	 pieces are indeed different
	*/
	...
	//Just cos90, sin90 for sake of readability
	private static final int cos90 = 0;
	private static final int sin90 = 1;
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece curPiece = root;
		Piece nextPiece = curPiece.computeNextRotation();

		while (!nextPiece.equals(root)) {
			curPiece.next = nextPiece;

			curPiece = nextPiece;
			nextPiece = curPiece.computeNextRotation();
		}
